// Approval writer — §6 step 6. Single Postgres transaction:
// ledger rows + result row + fixture/tie status + approval APPROVED + zero-sum assert.
...
export async function approveProposal(
  ...
  const transfers = approval.computed_transfers as (Transfer & {
    fixture_id?: string; tie_id?: string; trophy_id?: string; description?: string;
  })[];
  ...
  if (approval.subject_type === 'FIXTURE') {
    await db.from('fixtures').update({ status: 'RECORDED' }).eq('id', approval.subject_id);
  }
